...
class Model(object):
    ...
    def load_data(self):
        ...
        self.logger.info('Document count [{}]:{}, [{}]:{}'.format('trian',len(self.data['train']),'test',len(self.data['test'])))
    ...
    def add_placeholder(self):
        ...
        self.sent_num=tf.placeholder(tf.int32,shape=[None,3],name='sent_num')#stores which sentences belong to which bags
    ...
    def create_feed_dict(self,batch,wLabels=True,dtype='train'):
        #创建feed_dict传入计算图
        X,Y,pos1,pos2,mask,sent_num=batch['X'],batch['Y'],batch['Pos1'],batch['Pos2'],batch['Mask'],batch['sent_num']
    ...
    def add_model(self):
        ...
        with tf.variable_scope('Embeddings') as scope:
            ...
            pos1_embeddings=tf.get_variable('pos1_embeddings',[self.max_pos,self.params.pos_dim],initializer=tf.contrib.layers.xavier_initializer(),trainable=True,regularizer=self.regularizer)
            pos2_embeddings=tf.get_variable('pos2_embeddings',[self.max_pos,self.params.pos_dim],initializer=tf.contrib.layers.xavier_initializer(),trainable=True,regularizer=self.regularizer)
        ...
        with tf.variable_scope('pcnn') as scope:
            x=tf.layers.conv1d(inputs=embeds,filters=self.params.cnn_dim,kernel_size=3,strides=1,padding='same',kernel_initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.variable_scope('word_attention') as scope:
            word_query=tf.get_variable('word_query',[cnn_out_dim,1],initializer=tf.contrib.layers.xavier_initializer())
                ...
                tf.matmul(
                    tf.reshape(
                        tf.nn.softmax(
                            tf.reshape(
                                tf.matmul(
                                    tf.tanh(x),word_query
                                ),[self.total_sents,self.seq_len]
                            )
                        ),[self.total_sents,1,self.seq_len]
                    ),x
                ),[self.total_sents,cnn_out_dim]
            )
        
        with tf.variable_scope('sentence_attention') as scope:
            sentence_query=tf.get_variable('sentence_query',[cnn_out_dim,1],initializer=tf.contrib.layers.xavier_initializer())

            def getSentenceAtt(num):
                num_sents=num[1]-num[0]
                bag_sents=sent_repre[num[0]:num[1]]

                sentence_att_weights=tf.nn.softmax(tf.reshape(tf.matmul(tf.tanh(bag_sents),sentence_query),[num_sents]))
        ...
        with tf.variable_scope('fully_connected_layer') as scope:
            w=tf.get_variable('w',[cnn_out_dim,self.num_class],initializer=tf.contrib.layers.xavier_initializer(),regularizer=self.regularizer)
            b=tf.get_variable('b',initializer=np.zeros([self.num_class]).astype(np.float32),regularizer=self.regularizer)
    ...
    def run_epoch(self,sess,data,epoch,shuffle=True):
        ...
        for step,batch in enumerate(self.getBatches(data,shuffle)):
            ...
            summary_str,loss,accuracy,_=sess.run([self.merged_summary,self.loss,self.accuracy,self.train_op],feed_dict=feed)
            losses.append(loss)
            ...
            if step%10==0:
                self.logger.info('E:{} Train Accuracy({}/{}):\t{:.5}\t{:.5}\t{}\t{:.5}'.format(epoch,bag_count,len(self.data['train']),np.mean(accuracies)*100,np.mean(losses),self.params.name,self.best_train_acc))
    ...
    def predict(self,sess,data,wLabels=True,shuffle=False,label='Evaluating on test'):
        ...
        for step,batch in enumerate(self.getBatches(data,shuffle)):
            ...
            if  step%100==0:
                self.logger.info('{} ({}/{}) :\t{:.5}\t{:.5}\t{}'.format(label,bag_cnt,len(self.data['test']),np.mean(accuracies)*100,np.mean(losses),self.params.name))
    ...
    def fit(self,sess):
        ...
        if not self.params.only_eval:
            ...
            for epoch in range(self.params.max_epoches):
                train_loss,train_acc=self.run_epoch(sess,self.data['train'],epoch)
                self.logger.info('[Epoch {}]:Training Loss: {:.5},Training Accuracy:{:.5}\n'.format(epoch,train_loss,train_acc))
        ...
        self.logger.info('Final results:Prec:{} | Recall:[} | F1:{} | Area:{}'.format(test_prec,test_recall,test_f1,area_pr))
        #store predictions
        pickle.dump({'logit list':logit_list,'y_hot':y_hot},open('results/{}/prec_recall.pkl'.format(self.params.name),'wb'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data',dest='dataset',required=True,default='./riedel_processed.pkl',help='dataset to use')
    ...
    parser.add_argument('-l2',dest='l2',default=0.001,dtype=float,help='l2 regularization')
    parser.add_argument('-embed_loc',dest='embed_loc',default='./glove/glove.6B.50d_word2vec.txt',help='embed location')
    ...
    parser.add_argument('-lr',dest='lr',default=0.001,dtype=float,help='learning rate')
    parser.add_argument('-only_eval',dest='only_eval',action='store_true',help='Only evaluate pretrained model(skip training')
    parser.add_argument('-max_epochs',dest='max_epochs',default=2,dtype=int,help='Max epochs')
    parser.add_argument('-eps',dest='eps',default=0.00000001,dtype=float,help='value of epsilon')
